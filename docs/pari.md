...
## Сводка выплат в чате

Как только расчет по пари доведен до конца, бот публикует в том же канале сводку —
**ответом на сообщение-опрос**, чтобы итоги было видно рядом с самим пари:

- заголовок и статусный цвет повторяют пари, в описании — победивший вариант,
  итоговый коэффициент, призовой фонд и удержанная комиссия;
- поле «🏆 Победители» — участники победившего варианта, отсортированные по выплате;
- поле «💀 Проигравшие» — остальные;
- вместо обоих полей показывается «↩️ Возврат ставок», если пари отменено или на
  победивший вариант никто не поставил;
- строка участника: `@упоминание — ставка → выплата (изменение баланса)`, например
  `@user — 300 LP → 570 LP (+270)`.

Участники отмечаются упоминанием (`<@id>`) — внутри эмбеда оно рендерится как ник,
но не рассылает уведомления, поэтому сводка не спамит пингами. Ответ на сообщение пари
отправляется с `failOnInvalidReply(false)`: если опрос удалили, сводка все равно уйдет
в канал обычным сообщением.

Список участников ограничен лимитом поля эмбеда (1024 символа): что не поместилось,
сворачивается в строку «…и ещё N участников».

Сводка отправляется **ровно один раз**. Право на публикацию берется атомарным
`UPDATE paris SET results_posted_at = now() WHERE id = ? AND results_posted_at IS NULL`,
поэтому кнопка «Завершить» и восстановительный планировщик, запустившие расчет один
за другим, не продублируют сообщение. Отметка ставится только после того, как канал
найден: недоступный канал не «съедает» право на публикацию. Пари без единой ставки
сводку не порождает.

Ошибки отправки только логируются: доставка сообщения не влияет на движение средств.
...
После того как незакрытых ставок не осталось, `settle` проставляет `settled_at` и
публикует сводку выплат в канал.
