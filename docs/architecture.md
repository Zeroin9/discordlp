...
- **веб-дашборд** (Spring MVC + Thymeleaf) — таблица участников с балансами и временем
  в голосовых каналах на `/dashboard`.
...
- расчеты, которые нельзя повторять (выплаты по пари), опираются на заранее зафиксированные
  итоги и помечают обработанные записи флагом.
